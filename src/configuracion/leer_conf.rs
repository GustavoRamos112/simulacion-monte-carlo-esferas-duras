...
pub fn cargar_conf_usuario() -> ConfUsuario {
  let data =
    fs::read_to_string("resources\\conf.json").expect("No existe el archivo de configuracion");

  let mut conf_u: ConfUsuario =
    serde_json::from_str(&data).expect("El json no esta formateado correctamente");
  ...
  if !Path::new(&conf_u.dir_graficas).exists() {
    match fs::create_dir_all(&conf_u.dir_graficas) {
      ...
      Err(e) => {
        if conf_u.color {
          println!(
            "No se pudo crear el directorio para graficas: {}",
            e.to_string().red()
          );
        } else {
  ...
  if !Path::new(&conf_u.dir_pdb).exists() {
    match fs::create_dir_all(&conf_u.dir_pdb) {
      ...
      Err(e) => {
        if conf_u.color {
          println!(
            "No se pudo crear el directorio pdb: {}",
            e.to_string().red()
          );
        } else {
  ...
  if !Path::new(&conf_u.dir_dat).exists() {
    match fs::create_dir_all(&conf_u.dir_dat) {
      ...
      Err(e) => {
        if conf_u.color {
          println!(
            "No se pudo crear el directorio datos: {}",
            e.to_string().red()
          );
        } else {
