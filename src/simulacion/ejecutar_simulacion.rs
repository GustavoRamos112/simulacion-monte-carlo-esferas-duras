...
fn imprimir_tabla(conf: &Configuracion, conf_u: &ConfUsuario) {
  ...
  builder.push_record(["Intervalo de impresion", conf_u.intervalo_print.to_string().as_str()]);
  builder.push_record(["Ratio de actualizacion", conf_u.i_ratio.to_string().as_str()]);
  builder.push_record(["Frecuencia de calcular G(R)", conf_u.ngr.to_string().as_str()]);
...
pub fn ejecutar_simulacion(
  ...
  pdb(&mut conf, format!("{}\\{}", conf_u.dir_pdb, conf_u.pdb_final).as_str(), &conf_u.sigmar);
  if conf_u.color {
    println!(
      "\n{}\n{}\n{}",
      "┌───────────────────────┐".green(),
